...
const db = {
  ...
  async replaceAll({ tournaments, decks }) {
    const database = await this.open();
    return new Promise((resolve, reject) => {
      const transaction = database.transaction(["tournaments", "decks"], "readwrite");
      ["tournaments", "decks"].forEach((storeName) => {
        const store = transaction.objectStore(storeName);
        store.clear();
        (storeName === "tournaments" ? tournaments : decks).forEach((item) => store.put(item));
      });
...
async function dataSet() {
  const [tournaments, decks] = await Promise.all([db.all("tournaments"), db.all("decks")]);
  return {
...
function renderHome({ tournaments }) {
  ...
  app.innerHTML = `
    ...
    <section class="stats-grid" aria-label="成績の概要">
      <article class="card stat-card"><span class="stat-label">総対戦数</span><strong class="stat-value">${stats.total}</strong><span class="stat-sub">記録済みラウンド</span></article>
...
function renderTournamentForm({ decks, tournaments }, id) {
  ...
  const opponents = [...new Set(flatMatches(tournaments).map((match) => match.opponent).filter(Boolean))].sort((a, b) => a.localeCompare(b, "ja"));
  app.innerHTML = `<section class="page-heading"><div><h1>${tournament ? "大会記録を編集" : "大会を記録"}</h1><p>結果の詳細はラウンドごとに追加できます。</p></div><a href="#tournaments" class="button">一覧に戻る</a></section>
    <form id="tournament-form" class="form-layout" novalidate>
      ...
      <section class="card form-card"><div class="matches-head"><div><h2>ラウンド結果</h2><span class="event-meta">対戦後に必要な項目だけ入力できます。</span></div><button type="button" class="button button-small" id="add-match">＋ ラウンドを追加</button></div><p id="match-error" class="form-error" role="alert" hidden></p><div id="match-list"></div><datalist id="opponent-options">${opponents.map((name) => `<option value="${escapeHtml(name)}"></option>`).join("")}</datalist></section>
    </form>`;
  ...
  document.querySelector("#add-match").addEventListener("click", () => addMatch({ round: matchList.children.length + 1 }));
  document.querySelector("#tournament-form").addEventListener("submit", async (event) => {
    ...
    const matches = [...matchList.querySelectorAll(".match-card")].map((card, index) => ({
      id: card.dataset.id || uuid(), round: Number(card.querySelector('[name="round"]').value) || index + 1,
    ...
    if (invalidMatches.length) {
      const rounds = invalidMatches.map(({ index }) => index + 1).join("、");
      const error = document.querySelector("#match-error");
      error.textContent = `ラウンド ${rounds}：勝敗と残りサイド数が一致していません。勝ちなら自分の残りサイド数が少なく、負けなら相手の残りサイド数が少なくなるよう修正してください。`;
      error.hidden = false;
...
function matchTemplate(match, number) {
  return `<article class="match-card" data-id="${escapeHtml(match.id || uuid())}"><button type="button" class="button button-icon button-small remove-match" aria-label="このラウンドを削除">×</button><p class="match-title">ラウンド ${number}</p><div class="match-grid">
    <div class="field"><label>ラウンド</label><input name="round" inputmode="numeric" type="number" min="1" max="30" value="${escapeHtml(match.round || number)}"></div>
...
function renumberMatches() {
  document.querySelectorAll(".match-card").forEach((card, index) => { card.querySelector(".match-title").textContent = `ラウンド ${index + 1}`; });
}
...
function renderAnalysis({ tournaments }) {
  ...
  app.innerHTML = `<section class="page-heading"><div><h1>戦績分析</h1><p>記録されたラウンドから傾向を確認します。</p></div></section>
...
function renderSettings({ decks, tournaments }) {
  const opponents = [...new Set(flatMatches(tournaments).map((match) => match.opponent).filter(Boolean))].sort((a, b) => a.localeCompare(b, "ja"));
  ...
  <article class="card setting-card"><h2>相手デッキの入力候補</h2><p>相手デッキは自由入力です。すでに入力した名前は大会編集画面で候補になります。</p><div class="deck-list">${opponents.length ? opponents.map((name) => `<span class="deck-chip">${escapeHtml(name)}</span>`).join("") : '<span class="event-meta">まだ入力候補はありません。</span>'}</div></article>
...
async function exportData() {
  const payload = { app: "pokeca-record-note", version: 1, exportedAt: new Date().toISOString(), tournaments: await db.all("tournaments"), decks: await db.all("decks") };
...
async function importData(event) {
  ...
  try {
    ...
    await db.replaceAll(payload); showToast("バックアップを復元しました"); render();
